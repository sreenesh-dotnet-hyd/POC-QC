...
import type { Sample, SamplePriority, SampleStatusType } from "../types/sample.types";

export default function SamplesTable({ samplesData, onEdit, onView, onDelete }:
     {samplesData: Sample[], onEdit: (sample: Sample)=> void, onView: (sample:Sample)=>void, onDelete: (sample:Sample)=>void}) {
            ...
            <tbody>
                {samplesData.map((s) => (
                    <tr key={s.sampleId} className="odd:bg-gray-50 even:bg-gray-20 transition">
                        ...
                        <td className="px-6 py-4">{s.patientName}</td>
                        ...
                        <td className=" py-4">
                            <div className="flex gap-2">
                                ...
                                <button onClick={()=>onView(s)} className="px-6 py-1 text-xs rounded-lg bg-[#101aca] cursor-pointer text-white hover:bg-[#0d16a8] transition">
                                    View
                                </button>

                                <button onClick={()=>onEdit(s)} className="px-6 py-1 text-xs rounded-lg bg-[#101aca] cursor-pointer text-white hover:bg-[#0d16a8] transition">
                                    Edit
                                </button>

                                <button onClick={()=>onDelete(s)} className="px-6 py-1 text-xs rounded-lg bg-[#ca1010] cursor-pointer text-white hover:bg-[#9e0e0e] transition">
                                    <RiDeleteBin6Line />
                                </button>
                            </div>
                        </td>
